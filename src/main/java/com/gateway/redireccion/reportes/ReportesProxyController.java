...
public class ReportesProxyController {
    ...
    public ResponseEntity<?> proxyReportes(HttpServletRequest request,
        ...
        String targetUrl = "http://localhost:8081/api/reportes" + originalPath;
        ...
        if (method == HttpMethod.DELETE) {
            ...
            if (!"admin".equalsIgnoreCase(rol)) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .contentType(MediaType.APPLICATION_JSON)
                        .body("{\"error\": \"Solo admin puede eliminar reportes\"}");
            }
        }
        ...
        if (method == HttpMethod.PUT) {
            ...
            if (!"admin".equalsIgnoreCase(rol)) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .contentType(MediaType.APPLICATION_JSON)
                        .body("{\"error\": \"Solo admin puede actualizar reportes\"}");
            }
        }
