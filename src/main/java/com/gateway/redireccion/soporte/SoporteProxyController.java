...
public class SoporteProxyController {
    ...
    public ResponseEntity<?> proxySoporte(HttpServletRequest request,
        ...
        String targetUrl = "http://localhost:8084/api/soporte" + originalPath;
        ...
        if (method == HttpMethod.DELETE) {
            ...
            if (!"admin".equalsIgnoreCase(rol)) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .contentType(MediaType.APPLICATION_JSON)
                        .body("{\"error\": \"Solo admin puede eliminar soporte\"}");
            }
        }
        ...
        if (method == HttpMethod.PUT) {
            ...
            if (!"admin".equalsIgnoreCase(rol)) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .contentType(MediaType.APPLICATION_JSON)
                        .body("{\"error\": \"Solo admin puede actualizar soporte\"}");
            }
        }
